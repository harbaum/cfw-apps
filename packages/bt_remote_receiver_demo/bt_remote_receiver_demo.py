...
from bt_remote_receiver import BtRemoteReceiver
...
class ConnectWidget(QWidget):
    ...
    def search(self):
        self.lbl.setText(QCoreApplication.translate("Scanner", "Searching for BT remote receiver."))
    ...
    def search_now(self):
        # check for devices
        devs = BtRemoteReceiver.scan_for_devices()
    ...
    def connect(self, dev):
        self.sc = BtRemoteReceiver(dev)

        # display connecting message
        self.lbl.setText(QCoreApplication.translate("Scanner", "Connecting to BT remote receiver."))
...
class ControlWidget(QWidget):
    ...
    def __init__(self, sc, parent=None):
        ...
        info_lbl = QLabel("SW:" + info["sw"] + "  HW:" + info["hw"] )
    ...
    def stop(self):
        ...
        self.sc.setOutputs( { "M1": 0, "M2": 0, "M3": 0, "M4": 0 } )
        self.sc = None

class MainWindow(TouchWindow):
    def __init__(self):
        TouchWindow.__init__(self, QCoreApplication.translate("Main","BT remote"))
    ...
    def on_connected(self, dev):
        ...
        config = configparser.ConfigParser()
        config.add_section('bt_remote_receiver')
        config.set('bt_remote_receiver','bdaddr', dev.dev)
...
class FtcGuiApplication(TouchApplication):
    def __init__(self, args):
        ...
        path = os.path.dirname(os.path.realpath(__file__))
        translator.load(QLocale.system(), os.path.join(path, "bt_remote_receiver_demo_"))
